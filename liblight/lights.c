			...
			struct light_state_t const* state)
{
	...
	/*
	 * CM7 uses hard-coded values for charging led colors.
	 * Green is used for "Charging Full", so a special case is needed.
	 * If red and blue are both off, set blue to green.
	 */
	if ((red->enabled == 0) && (blue->enabled == 0))
		blue->enabled   = !!(color >> 8 & 0xff);

	LOGD("comp_led_states: red=(%u, %d, %d), blue=(%u, %d, %d).",
